...
function getDirectReply(utterance: string): string | null {
  ...
  if (regionOnly) {
    return `${normalized.replace(/[.!?]$/, "")} 지역 확인했습니다. 문제 부위 전체 사진과 가까이 찍은 사진을 함께 보내주시면 확인 가능한 범위부터 안내드리겠습니다.`;
  }

  if (/^(사진|사진입니다|사진 보냈어요|사진 보냈습니다|첨부했어요|첨부했습니다|보냈어요|보냈습니다)[.!?]?$/.test(normalized)) {
    return "사진을 보내주신 경우 확인 가능한 범위부터 살펴보겠습니다. 아직 지역을 안 보내주셨다면 지역도 함께 알려주세요. 사진만으로 확정 진단은 어렵고, 필요하면 상담원이 추가 확인드리겠습니다.";
  }
